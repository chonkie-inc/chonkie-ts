import { LateChunker } from '../../chonkie/chunker/late';
import { Tokenizer } from '../../chonkie/tokenizer';
import { LateChunk } from '../../chonkie/types/late';
...
describe('LateChunker', () => {
  ...
  it('should initialize correctly with model name string', async () => {
    const chunker = await LateChunker.create('google-bert/bert-base-uncased');
    expect(chunker).toBeDefined();
    expect(chunker.chunkSize).toBe(512);
    expect(chunker.returnType).toBe('chunks');
  });

  it('should throw error for invalid embedding model type', async () => {
    await expect(async () => {
      await LateChunker.create(123 as any);
    }).rejects.toThrow('Invalid embedding model type');
  });
  ...
  it('should chunk text correctly', async () => {
    ...
    expect(chunks[0]).toBeInstanceOf(LateChunk);
    ...
    chunks.forEach(chunk => {
      ...
      expect((chunk as LateChunk).embedding).toBeDefined();
      expect(Array.isArray((chunk as LateChunk).embedding)).toBe(true);
    });
  });
  ...
  it('should handle short text', async () => {
    ...
    expect(chunk).toBeInstanceOf(LateChunk);
  ...
  it('should have correct string representation', async () => {
    ...
    expect(representation).toContain(`embedding=[`);
  });
  ...
  it('should work with custom rules', async () => {
    ...
    const chunker = await LateChunker.create('google-bert/bert-base-uncased', 512, rules);
    const chunks = await chunker.chunk(sampleText) as Chunk[];
