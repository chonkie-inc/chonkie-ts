...
export class Hubbie {
     ...
     * @param lang - The language of the recipe to get
    ...
    public async getRecipe(
        ...
        if ((!name || !lang) && !filePath) {
            throw new Error("Either (name & lang) or path must be provided.");
        }

        let recipeContent: string;

        // If path is not provided, download the recipe from the hub
        if (!filePath && name && lang) {
            try {
                const recipeBlob = await downloadFile({
                    ...
                    path: `${this.getRecipeConfig.subfolder}/${name}_${lang}.json`,
                });

                if (!recipeBlob) {
                    throw new Error(`Could not download recipe '${name}_${lang}'`);
                }

                recipeContent = await recipeBlob.text();
            } catch (error) {
                throw new Error(`Could not download recipe '${name}_${lang}'. Ensure name and lang are correct or provide a valid path. Error: ${error}`);
            }
        } else {
